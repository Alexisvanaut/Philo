...
/*   Updated: 2025/06/30 14:52:17 by alvanaut         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_args(int argc, char **argv, t_data *data)
{
	...
	{
		fprintf(stderr,
			"Usage:%s nbr_philo time_to_die time_to_sleep [limit_meals]\n",
			argv[0]);
		return (1);
	}
	data->nbr_philo = ft_atoi_positive(argv[1]);
	data->time_to_die = ft_atoi_positive(argv[2]);
	data->time_to_sleep = ft_atoi_positive(argv[3]);
	data->limit_meals = -1;
	if (argc == 6)
		data->limit_meals = ft_atoi_positive(argv[4]);
	...
	{
		fprintf(stderr, "limit_meals must be positive integer if provided\n");
		return (1);
	}
	return (0);
}
